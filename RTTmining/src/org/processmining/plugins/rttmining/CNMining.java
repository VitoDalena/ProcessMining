...
public class CNMining
{
  ...
  private double relative_to_best = 0.0D;
  ...
  public double[] computePrecisionRecall(boolean[][] testMatrix, boolean[][] realMatrix)
  {
    ...
    int r = testMatrix.length;
    int c = testMatrix[0].length;
    for (int i = 0; i < r; i++) {
      for (int j = 0; j < c; j++)
      {
        boolean estimated = testMatrix[i][j];
        boolean real = realMatrix[i][j];
        if (estimated)
        {
          if (real) {
            TP++;
    ...
    throws Exception
  {
    ...
    final NiceSlider slider = SlickerFactory.instance().createNiceIntegerSlider("T percentage", 0, 
    ...
    ChangeListener listener = new ChangeListener()
    {
      public void stateChanged(ChangeEvent e)
      {
        ...
        CNMining.this.value = (percentage / 100.0D);
      }
    };
    slider.addChangeListener(listener);
    listener.stateChanged(null);
    
    final NiceSlider slider1 = SlickerFactory.instance().createNiceIntegerSlider("�� percentage", 0, 100, 
    ...
    ChangeListener listener1 = new ChangeListener()
    {
      public void stateChanged(ChangeEvent e)
      {
        ...
        CNMining.this.delta = (percentage / 100.0D);
      }
    };
    slider1.addChangeListener(listener1);
    listener1.stateChanged(null);
    
    final NiceSlider slider2 = SlickerFactory.instance().createNiceIntegerSlider("\034T_r2b percentage", 0, 
    ...
    ChangeListener listener2 = new ChangeListener()
    {
      public void stateChanged(ChangeEvent e)
      {
        ...
        CNMining.this.relative_to_best = (percentage / 100.0D);
      }
    };
    ...
    s.setSigmaLogNoise(this.value);
    s.setLogName(logName);
    s.setFallFactor(this.delta);
    s.setRelativeToBest(this.relative_to_best);
    ...
    throws Exception
  {
        ...
        else
        {
          ...
          for (int i = 0; i < constraints.size(); i++)
          {
            ...
            else
            {
              ...
              for (Iterator localIterator1 = constr.getBodyList().iterator(); localIterator1.hasNext(); localIterator2.hasNext())
              {
                String body = (String)localIterator1.next();
                localIterator2 = constr.getHeadList().iterator(); continue;String head = (String)localIterator2.next();
    ...
    if (enable_constraints)
    {
      ...
      cnm.buildPG0(graph, m, vincoli_positivi_unfolded, vincoli_positivi, vincoli_negati_unfolded, vincoli_negati, lista_forbidden, 
        lista_forbidden_unfolded, map, (ObjectObjectOpenHashMap)attivita_tracce, traccia_attivita, csm, sigma_low_cs_constr_edges, 
    ...
    ObjectArrayList<FakeDependency> attivita_parallele = cnm.getAttivitaParallele(m, graph, map, vincoli_positivi, 
    ...
    cnm.algoritmo2(m, graph, map, (ObjectObjectOpenHashMap)attivita_tracce, traccia_attivita, csm, sigma_up_cs_diff, folded_map, 
    ...
    ObjectArrayList<FakeDependency> attivita_parallele_residue = cnm.getAttivitaParallele(m, graph, map, 
    ...
    for (;;)
    {
      ObjectArrayList<Edge> removableEdges = cnm.removableEdges(folded_g, csmOri, vincoli_positivi, folded_map, 
        relative_to_best);
  ...
  private static ProMJGraph buildJGraph(DirectedGraph<?, ?> causalNet)
  {
    ...
    layout.setParallelEdgeSpacing(15.0D);
    layout.setFixRoots(false);
    
    layout.setOrientation(((Integer)map.get(causalNet, "ProM_Vis_attr_orientation", Integer.valueOf(5))).intValue());
  ...
  public Graph rimuoviAttivitaFittizie(Graph folded_g, ObjectIntOpenHashMap<String> folded_map, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivita, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> attivita_traccia, Node start, Node end, XLog log, ObjectArrayList<Node> startActivities, ObjectArrayList<Node> endActivities)
  {
    ...
    for (ObjectCursor<Node> n : startActs) {
      if (((Node)n.value).getOuter_degree() > 0)
      {
        Node cn = cleanG.getNode(((Node)n.value).getNomeAttivita(), folded_map.get(((Node)n.value).getNomeAttivita()));
  ...
  public boolean[][] generaAdjacentsMatrix(Graph folded_g)
  {
    ...
    for (int i = 0; i < folded_g.listaNodi().size(); i++)
    {
      ...
      for (int j = 0; j < folded_g.adjacentNodes(n).size(); j++)
      {
        ...
        adjacentsMatrix[n.getID_attivita()][adjacent.getID_attivita()] = 1;
      }
    }
  ...
  public void aggiungiAttivitaFittizia(XLog xlog)
  {
    ...
    for (int i = 0; i < xlog.size(); i++)
    {
      ...
      String concept_name = activity_first.getAttributes().get("concept:name");
  ...
  public void bestEdge(Graph unfolded_g, double[][] m, ObjectArrayList<Constraint> lista_vincoli_positivi_unfolded, ObjectArrayList<Constraint> lista_vincoli_positivi_folded, ObjectArrayList<Constraint> vincoli_negati, ObjectArrayList<Forbidden> lista_forbidden, ObjectArrayList<Forbidden> lista_forbidden_unfolded, ObjectIntOpenHashMap<String> map, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> attivita_tracce, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivita, double[][] csm, double sigma, Graph folded_g, ObjectIntOpenHashMap<String> folded_map)
  {
    ...
    for (int i = 0; i < lista_vincoli_positivi_unfolded.size(); i++)
    {
      ...
      if (!vincolo.isPathConstraint())
      {
        ...
        Iterator localIterator2;
        for (Iterator localIterator1 = vincolo.getBodyList().iterator(); localIterator1.hasNext(); localIterator2.hasNext())
        {
          String body = (String)localIterator1.next();
          
          String activity_x = body;
          
          localIterator2 = vincolo.getHeadList().iterator(); continue;String head = (String)localIterator2.next();
  ...
  public void bestPath(Graph unfolded_g, double[][] m, ObjectArrayList<Constraint> lista_vincoli_positivi_unfolded, ObjectArrayList<Constraint> lista_vincoli_positivi_folded, ObjectArrayList<Constraint> vincoli_negati, ObjectArrayList<Forbidden> lista_forbidden, ObjectArrayList<Forbidden> lista_forbidden_unfolded, ObjectIntOpenHashMap<String> map, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> attivita_tracce, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivita, double[][] csm, double sigma, Graph folded_g, ObjectIntOpenHashMap<String> folded_map)
  {
    ...
    for (int i = 0; i < lista_vincoli_positivi_unfolded.size(); i++)
    {
      ...
      if (vincolo.isPathConstraint())
      {
        ...
        for (localIterator1 = vincolo.getBodyList().iterator(); localIterator1.hasNext(); localIterator2.hasNext())
        {
          String body = (String)localIterator1.next();
  ...
  public void algoritmo2(double[][] m, Graph graph, ObjectIntOpenHashMap<String> map, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> attivita_tracce, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivita, double[][] csm, double sigma_1, ObjectIntOpenHashMap<String> folded_map, ObjectArrayList<Forbidden> lista_forbidden, ObjectArrayList<Constraint> vincoli_positivi, ObjectArrayList<Constraint> vincoli_negati)
  {
    ...
    for (;;)
    {
      ...
      ObjectArrayList<FakeDependency> attivita_parallele = getAttivitaParallele(m, graph, map, vincoli_positivi, 
        folded_map, folded_g);
      ...
      lista_candidati_best_pred = bestPred_Folded(ny.getID_attivita(), nx.getID_attivita(), map, attivita_tracce, 
        traccia_attivita);
      ...
      if (lista_candidati_best_pred != null) {
        if (lista_candidati_best_pred.size() > 0)
        {
          ...
          for (int i = 0; i < lista_candidati_best_pred.allocated.length; i++) {
            if (lista_candidati_best_pred.allocated[i] != 0)
            {
          ...
          best_pred = getFinalBestPred(graph, csm, ny, map, lista_candidati_best_pred_unfolded, 
      ...
      lista_candidati_best_succ = bestSucc_Folded(best_ap.getAttivita_x(), best_ap.getAttivita_y(), map, 
      ...
      if (lista_candidati_best_succ != null) {
        if (lista_candidati_best_succ.size() > 0)
        {
          ...
          best_succ = getFinalBestSucc(graph, csm, nx, map, lista_candidati_best_succ_unfolded, 
            vincoli_negati, lista_forbidden, folded_g, folded_map, false);
        }
        else
        {
      ...
      if (graph.isConnected(ny, nx))
      {
        ...
        if (soddisfa_vincoli_positivi)
        {
          ...
          lista_candidati_best_pred_yx = bestPred_Folded(nx.getID_attivita(), ny.getID_attivita(), map, 
            attivita_tracce, traccia_attivita);
          ...
          if (lista_candidati_best_pred_yx != null) {
            if (lista_candidati_best_pred_yx.size() > 0)
            {
              ...
              best_pred_yx = getFinalBestPred(graph, csm, nx, map, lista_candidati_best_pred_unfolded, 
                vincoli_negati, lista_forbidden, folded_g, folded_map, false);
            }
            else
            {
          ...
          lista_candidati_best_succ_yx = bestSucc_Folded(best_ap.getAttivita_y(), best_ap.getAttivita_x(), 
          ...
          if (lista_candidati_best_succ_yx != null) {
            if (lista_candidati_best_succ_yx.size() > 0)
            {
              ...
              best_succ_yx = getFinalBestSucc(graph, csm, ny, map, lista_candidati_best_succ_unfolded, 
                vincoli_negati, lista_forbidden, folded_g, folded_map, false);
            }
            else
            {
  ...
  private boolean bfs(Graph graph, Node x, Node y, Node f, ObjectArrayList<Node> path)
  {
    ...
    Node t;
    int i;
    for (; !nodes.isEmpty(); i < graph.adjacentNodes(t).size())
    {
  ...
  public double[][] buildNextMatrix(XLog log, ObjectIntOpenHashMap<String> map, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivita)
  {
    ...
    for (int i = 0; i < traccia_attivita.allocated.length; i++) {
      if (traccia_attivita.allocated[i] != 0)
      {
  ...
  public double[][] calcoloMatriceDeiCausalScore(XLog log, ObjectIntOpenHashMap<String> map, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivita, double ff)
  {
    ...
    try
    {
      ...
      WeightEstimator weightEstimator = new WeightEstimator(map.size(), -1, ff, 
        1);
  ...
  private boolean checkTrueEdge_CS2(Node n, Node adjacent_i, ObjectArrayList<Node> adjacents, double[][] csmOri, Graph g, ObjectIntOpenHashMap<String> map, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> attivita_tracce, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivita, double sigma_1, double sigma_2)
  {
    ...
    sigma_1 = 1.0D;
    if (csmOri[n.getID_attivita()][adjacent_i.getID_attivita()] - csmOri[adjacent_i.getID_attivita()][n.getID_attivita()] <= sigma_1)
    {
      remove_edge = true;
      
      ObjectOpenHashSet<String> candidatiZ = new ObjectOpenHashSet();
      for (ObjectCursor<Node> e : g.listaNodi()) {
        if ((g.isConnected((Node)e.value, adjacent_i)) && (!((Node)e.value).getNomeAttivita().equals(n.getNomeAttivita()))) {
          candidatiZ.add(((Node)e.value).getNomeAttivita());
        }
      }
      ObjectOpenHashSet<String> tracce_n = new ObjectOpenHashSet((ObjectContainer)attivita_tracce.get(n.getNomeAttivita()));
      Object tracce_adj = new ObjectArrayList((ObjectContainer)attivita_tracce.get(adjacent_i.getNomeAttivita()));
      
      ((ObjectArrayList)tracce_adj).retainAll(tracce_n);
      if (((ObjectArrayList)tracce_adj).size() == 0)
      {
        remove_edge = false;
      }
      else
      {
        ObjectOpenHashSet<String> candidatiW = new ObjectOpenHashSet();
        for (ObjectCursor<Node> na : adjacents) {
          candidatiW.add(((Node)na.value).getNomeAttivita());
        }
        int counter = 0;
        
        boolean autoanello_y = false;
        if (g.isConnected(adjacent_i, adjacent_i)) {
          autoanello_y = true;
        }
        boolean autoanello_x = false;
        if (g.isConnected(n, n)) {
          autoanello_x = true;
        }
        for (ObjectCursor<String> traccia : (ObjectArrayList)tracce_adj) {
          if ((!esisteAttivatore((String)traccia.value, adjacent_i.getNomeAttivita(), adjacent_i.getNomeAttivita(), traccia_attivita, candidatiZ, false, autoanello_y, false)) || 
          
            (!esisteAttivatore((String)traccia.value, n.getNomeAttivita(), n.getNomeAttivita(), traccia_attivita, candidatiW, false, autoanello_x, true)))
          {
            counter++;
            if (counter > sigma_2 * ((ObjectArrayList)tracce_adj).size())
            {
              remove_edge = false;
              System.out.println("traccia non soddisfatta " + traccia);
              break;
            }
          }
        }
  ...
  {
    int i;
    for (Iterator localIterator = vincoli_positivi.iterator(); localIterator.hasNext(); i < map.allocated.length)
    {
  ...
  public ObjectArrayList<FakeDependency> getAttivitaParallele(double[][] m, Graph graph, ObjectIntOpenHashMap<String> map, ObjectArrayList<Constraint> vincoli_positivi, ObjectIntOpenHashMap<String> folded_map, Graph folded_g)
  {
    ...
    Iterator localIterator2;
    for (Iterator localIterator1 = graph.listaNodi().iterator(); localIterator1.hasNext(); localIterator2.hasNext())
    {
      ObjectCursor<Node> np = (ObjectCursor)localIterator1.next();
      
      localIterator2 = graph.adjacentNodes((Node)np.value).iterator(); continue;ObjectCursor<Node> nr = (ObjectCursor)localIterator2.next();
  ...
  public Graph getGrafoAggregato(Graph g, XLog log, boolean flag, ObjectIntOpenHashMap<String> mapOri, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> attivita_tracceOri, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivitaOri)
  {
    if (flag)
    {
      ...
      for (int i = 0; i < log.size(); i++)
      {
        ...
        {
          String nome_attivita = activity.getAttributes().get("concept:name");
  ...
  {
    ObjectArrayList<Constraint> unsat_constr_list = new ObjectArrayList();
    for (int i = 0; i < vincoli_negati.size(); i++)
    {
      Constraint constr = (Constraint)vincoli_negati.get(i);
      if (constr.isPathConstraint())
      {
        for (ObjectCursor<Node> e : folded_g.listaNodi()) {
          ((Node)e.value).setMark(false);
        }
        Iterator localIterator2;
        for (??? = constr.getHeadList().iterator(); ???.hasNext(); localIterator2.hasNext())
        {
          String head = (String)???.next();
          
          localIterator2 = constr.getBodyList().iterator(); continue;String body = (String)localIterator2.next();
          if (folded_g.isConnected(folded_g.getNode(body, folded_map.get(body)), folded_g.getNode(head, folded_map.get(head))))
          {
            unsat_constr_list.add(constr);
            break;
          }
          if (bfs(folded_g, folded_g.getNode(body, folded_map.get(body)), folded_g.getNode(head, folded_map.get(head)), null, null))
          {
            unsat_constr_list.add(constr);
  ...
  public void postProcessing_dip_indirette(Graph g, ObjectIntOpenHashMap<String> map, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> attivita_tracce, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivita, double[][] cs, double sigma_2, ObjectArrayList<Constraint> vincoli_positivi)
  {
    ...
    int cursor;
    for (Iterator localIterator1 = g.listaNodi().iterator(); localIterator1.hasNext(); cursor < adjacents.size())
    {
  ...
  public void postProcessing_paralleli(Graph g, double[][] csmOri, ObjectIntOpenHashMap<String> map, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> attivita_tracce, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivita, double sigma_up_cs_diff, double sigma_log_noise, ObjectArrayList<Constraint> vincoli_positivi)
  {
    ObjectArrayList<Node> adjacents;
    int it;
    for (Iterator localIterator = g.listaNodi().iterator(); localIterator.hasNext(); it < adjacents.size())
    {
      ObjectCursor<Node> nn = (ObjectCursor)localIterator.next();
      Node n = (Node)nn.value;
      ...
      Node adjacent_i = (Node)adjacents.get(it);
      if ((n.getOuter_degree() > 1) && (adjacent_i.getInner_degree() > 1))
      {
        if (!verificaVincoliPositivi(g, n, adjacent_i, vincoli_positivi, map))
        {
        ...
        if ((checkTrueEdge_CS2(n, adjacent_i, adjacents, csmOri, g, map, attivita_tracce, traccia_attivita, sigma_up_cs_diff, sigma_log_noise)) && 
          (follows(n.getNomeAttivita(), adjacent_i.getNomeAttivita(), traccia_attivita, attivita_tracce, sigma_log_noise)))
        {
          g.removeEdge(n, adjacent_i);
  ...
  public boolean verificaVincoliPositivi(Graph graph, Node np, Node nr, ObjectArrayList<Constraint> vincoli_positivi, ObjectIntOpenHashMap<String> map)
  {
    ...
    for (ObjectCursor<Constraint> cc : vincoli_positivi)
    {
      ...
      for (Iterator localIterator2 = c.getHeadList().iterator(); localIterator2.hasNext(); localIterator3.hasNext())
      {
        String head = (String)localIterator2.next();
        
        Node nHead = graph.getNode(head, map.get(head));
        
        localIterator3 = c.getBodyList().iterator(); continue;String body = (String)localIterator3.next();
  ...
  public void computeBindings(Graph g, ObjectObjectOpenHashMap<String, ObjectArrayList<String>> traccia_attivita, ObjectIntOpenHashMap<String> map)
  {
    ...
    for (int it1 = 0; it1 < traccia_attivita.allocated.length; it1++) {
      if (traccia_attivita.allocated[it1] != 0)
      {
