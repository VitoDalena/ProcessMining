 ...
 * 
 */
@Plugin(name = "Visualize Petri net", returnLabels = { "Visualized Petrinet" }, returnTypes = { JComponent.class }, parameterLabels = {
		"Petri net", "Initial Marking" }, userAccessible = false)
@Visualizer
public class PetriNetVisualization {

	private JComponent visualizeWithAll(PluginContext context, PetrinetGraph n, Marking m) {
		...
		try {
			ListSelectionPanel view = new ListSelectionPanel("Non-relaxed-sound cause", "Not fireable transitions", true);
			for (NonRelaxedSoundTransitionsConnection comp : cm.getConnections(NonRelaxedSoundTransitionsConnection.class, context, n)) {
				NonRelaxedSoundTransitionsSet component = comp.getObjectWithRole(NonRelaxedSoundTransitionsConnection.TRANSITIONS);
				Semantics<Marking, Transition> semantics = Cast.<Semantics<Marking, Transition>>cast(comp
						.getObjectWithRole(AbstractSemanticConnection.SEMANTICS));
				for (SortedSet<Transition> sequence : component) {
					view.addElementCollection(sequence, "<html>" + sequence.toString() + " (" + semantics.toString()
	...
	private static void addInvariantSet(String name, String label,
		...
		try {
			ListSelectionPanel view = new ListSelectionPanel(name, label, true);
			for (AbstractInvariantMarkingConnection connection : context.getConnectionManager().getConnections(
					clazz, context, n)) {
