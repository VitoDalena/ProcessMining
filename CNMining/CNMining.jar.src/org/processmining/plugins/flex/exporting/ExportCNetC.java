 ...
 *
 */
@Plugin(name = "Export as Causal Net extended with cancelation region (.cnetc)", returnLabels = {}, returnTypes = {}, parameterLabels = { "Causal net", "File" }, userAccessible = true)
@UIExportPlugin(description = "Export as Causal Net extended with cancelation region (.cnetc)", extension = "cnetc")
public class ExportCNetC {
	@PluginVariant(variantLabel = "Export as Causal Net extended with cancelation region (.cnetc)", requiredParameterLabels = { 0, 1 })
	public void exportCNet(UIPluginContext context, Flex net, File file) throws IOException {
		exportCNetToCNetFile(context, net, file);
	}

	private void exportCNetToCNetFile(UIPluginContext context, Flex net, File file) throws IOException {
		...
		// get cancellation region
		CancellationRegion cancellationRegion = null;
		try {
			Collection<FlexCancellationRegionConnection> conns = connManager.getConnections(
					FlexCancellationRegionConnection.class, context, net);
			if (conns != null) {
				cancellationRegion = conns.iterator().next()
		...
		String text = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>"
				+ createStringRep(net, startTaskNodesSet, endTaskNodesSet, cancellationRegion);
	...
	private String createStringRep(Flex net, StartTaskNodesSet startTaskNodesSet, EndTaskNodesSet endTaskNodesSet,
		...
		if (cancellationRegion != null) {
			for (FlexNode node : cancellationRegion.keySet()) {
				sb.append("<cancellationRegionNode id=\"");
				sb.append(mapFlexNodeToId.get(node));
				sb.append("\">");
				for (Pair<FlexNode, FlexNode> pairNode : cancellationRegion.get(node)) {
					sb.append("<arc id=\"");
					sb.append(mapArcToId.get(pairNode));
					sb.append("\" />");
				}
				for (FlexNode cancelledNode : cancellationRegion.getNodeCancellationFor(node)){
					sb.append("<node id=\"");
					sb.append(mapFlexNodeToId.get(cancelledNode));
					sb.append("\" />");
				}
				sb.append("</cancellationRegionNode>");
			}
		}
		sb.append("</cnet>");
		return sb.toString();
