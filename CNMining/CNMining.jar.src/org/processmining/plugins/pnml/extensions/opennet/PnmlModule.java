...
import org.processmining.plugins.pnml.PnmlNet;
...
public class PnmlModule extends PnmlElement {
	...
	protected boolean importElements(XmlPullParser xpp, Pnml pnml) {
		...
		if (xpp.getName().equals(PnmlFinalMarkings.TAG)) {
			finalMarkings = new PnmlFinalMarkings();
	...
	public void convertToOpenNet(OpenNet openNet, Marking marking, Pnml pnml, GraphLayoutConnection layout) {
		...
		if (net != null) {
			net.convertToNet(openNet, marking, placeMap, transitionMap, edgeMap, layout);
		}
		/*
	 ...
	 */
	public PnmlModule convertFromOpenNet(OpenNet openNet, Marking marking, Map<String, AbstractGraphElement> idMap, GraphLayoutConnection layout) {
		/*
		 ...
		 */
		net = new PnmlNet();
		net.convertFromNet(openNet, marking, map, 1, idMap, layout);
		/*
