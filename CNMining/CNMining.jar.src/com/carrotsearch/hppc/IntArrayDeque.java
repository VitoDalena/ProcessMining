...
/*  82 */     this.buffer = new int[initialCapacity];
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */     
/* 136 */     for (IntCursor cursor : container)
/*     */     {
...
/*     */   {
/* 151 */     int size = 0;
/* 152 */     for (IntCursor cursor : iterable)
/*     */     {
...
/*     */     
/* 202 */     for (IntCursor cursor : container)
/*     */     {
...
/*     */   {
/* 217 */     int size = 0;
/* 218 */     for (IntCursor cursor : iterable)
/*     */     {
...
/*     */     
/* 234 */     int result = this.buffer[this.head];
...
/* 249 */     int result = this.buffer[this.tail];
...
/*     */     {
/* 461 */       Arrays.fill(this.buffer, this.head, this.tail, 0);
/*     */     }
/*     */     else
/*     */     {
/* 465 */       Arrays.fill(this.buffer, 0, this.tail, 0);
/* 466 */       Arrays.fill(this.buffer, this.head, this.buffer.length, 0);
/*     */     }
...
/* 477 */     this.buffer = new int[this.resizer.round(5)];
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */ 
/*     */ 
/* 496 */       int[] newBuffer = new int[newSize];
/* 497 */       if (bufferLen > 0)
/*     */       {
...
/*     */   {
/* 514 */     int size = size();
/* 515 */     return toArray(new int[size]);
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */     try
/*     */     {
/* 557 */       IntArrayDeque cloned = (IntArrayDeque)super.clone();
/* 558 */       cloned.buffer = ((int[])this.buffer.clone());
/* 559 */       return cloned;
/*     */     }
...
/*     */     {
/* 595 */       this.cursor = new IntCursor();
/* 596 */       this.cursor.index = IntArrayDeque.oneLeft(IntArrayDeque.this.head, IntArrayDeque.this.buffer.length);
/* 597 */       this.remaining = IntArrayDeque.this.size();
/*     */     }
/*     */     
/*     */ 
...
/* 604 */         return (IntCursor)done();
/*     */       }
/* 606 */       this.remaining -= 1;
/* 607 */       this.cursor.value = IntArrayDeque.this.buffer[(this.cursor.index = IntArrayDeque.oneRight(this.cursor.index, IntArrayDeque.this.buffer.length))];
...
/*     */     {
/* 622 */       this.cursor = new IntCursor();
/* 623 */       this.cursor.index = IntArrayDeque.this.tail;
/* 624 */       this.remaining = IntArrayDeque.this.size();
/*     */     }
/*     */     
/*     */ 
/*     */     protected IntCursor fetch()
...
/* 631 */         return (IntCursor)done();
/*     */       }
/* 633 */       this.remaining -= 1;
/* 634 */       this.cursor.value = IntArrayDeque.this.buffer[(this.cursor.index = IntArrayDeque.oneLeft(this.cursor.index, IntArrayDeque.this.buffer.length))];
...
/* 877 */     if (obj != null)
/*     */     {
/* 879 */       if ((obj instanceof IntDeque))
/*     */       {
/* 881 */         IntDeque other = (IntDeque)obj;
/* 882 */         if (other.size() == size())
/*     */         {
/* 884 */           int fromIndex = this.head;
/* 885 */           int[] buffer = this.buffer;
/* 886 */           int i = fromIndex;
/* 887 */           for (IntCursor c : other)
/*     */           {
...
/*     */ 
/*     */ 
/*     */   public static IntArrayDeque newInstanceWithCapacity(int initialCapacity)
/*     */   {
/* 917 */     return new IntArrayDeque(initialCapacity);
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/*     */   {
/* 926 */     IntArrayDeque coll = new IntArrayDeque(elements.length);
...
/*     */ 
/*     */ 
/*     */   public static IntArrayDeque from(IntArrayDeque container)
/*     */   {
/* 937 */     return new IntArrayDeque(container);
/*     */   }
/*     */ }


/* Location:              /home/menick/Scrivania/ProcessMining/CNMining/CNMining.jar!/com/carrotsearch/hppc/IntArrayDeque.class
