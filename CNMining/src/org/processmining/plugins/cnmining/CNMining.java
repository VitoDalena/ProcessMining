...
/*      */   
/*   95 */   public static double value = 0.0D;
/*   96 */   public static double delta = 0.0D;
/*   97 */   public static double relative_to_best = 0.0D;
...
/*      */   @Plugin(
		...
		returnLabels={"CausalNet", 
				"StartTaskNodesSet", "EndTaskNodesSet", "CausalNetAnnotations"}, 
		returnTypes={Flex.class, StartTaskNodesSet.class, EndTaskNodesSet.class, CausalNetAnnotations.class}, 
		userAccessible=true, help="?? ")
/*      */
/*      */   @UITopiaVariant(
		affiliation="DIMES University of Calabria", author="F. Lupia", email="lupia@dimes.unical.it")
/*      */
...
/*      */         
/*  161 */         CNMining.value = (percentage / 100.0D);
/*      */ 
/*      */       }
/*      */       
/*      */ 
...
/*  177 */         CNMining.delta = (percentage / 100.0D);
/*      */       }
/*      */       
/*      */ 
/*  181 */     };
...
/*      */         
/*  193 */         CNMining.relative_to_best = (percentage / 100.0D);
/*      */ 
/*      */       }
/*      */       
/*      */ 
...
/*  220 */     s.setSigmaLogNoise(value);
/*  221 */     s.setLogName(logName);
/*  222 */     s.setFallFactor(delta);
/*  223 */     s.setRelativeToBest(relative_to_best);
/*      */     
...
/* 1332 */           if (obX.allocated[ts] != false) {
...
/* 1340 */           if (ibY.allocated[ts] != false) {
...
/* 1354 */           if (extendedObX.allocated[ts] != false) {
...
/* 1362 */           if (extendedIbY.allocated[ts] != false) {
...
/* 1426 */         if (extendedIbY.allocated[ts] != false) {
...
/* 1442 */         if (extendedObX.allocated[ts] != false) {
...
/* 1505 */         if (n.getOutput().allocated[ts] != false) {
...
/* 1519 */         if (n.getInput().allocated[ts] != false) {
...
/*      */ 
/*      */ 
/* 1811 */     for (ObjectCursor<Node> n1 : startActs) {
/* 1812 */       if (((Node)n1.value).getOuter_degree() > 0) {
/* 1813 */         Node cn = cleanG.getNode(((Node)n1.value).getNomeAttivita(), folded_map.get(((Node)n1.value).getNomeAttivita()));
...
/* 2011 */         adjacentsMatrix[n.getID_attivita()][adjacent.getID_attivita()] = true;
/*      */       }
/*      */     }
...
/*      */ 
/*      */ 
/* 2072 */       XAttribute concept_name = activity_first.getAttributes().get("concept:name");
/*      */       
/*      */ 
/*      */ 
/*      */ 
...
/* 2536 */             if (lista_candidati_best_pred.allocated[i] != false) {
...
/* 3231 */       if (traccia_attivita.allocated[i] != false)
/*      */       {
...
/* 3269 */       if (traccia_attivita.allocated[i] != false)
/*      */       {
...
/*      */     {
/* 3720 */       if (traccia_attivita.allocated[i] != false)
/*      */       {
...
/*      */ 
				...
				do
				{
					...
					/*      */       
					/* 4294 */       localIterator2 = graph.adjacentNodes((Node)np.value).iterator(); 
...
/*      */         Iterator localIterator5 = folded_g.listaNodi().iterator();
				   ...
				   while(localIterator5.hasNext() && localIterator6.hasNext())
				   {
					   ...
					   /*      */           }
					   /* 4456 */           boolean path_violated = bfs(folded_g, folded_g.getNode(ny.getNomeAttivita().split("#")[0], folded_map.get(ny.getNomeAttivita().split("#")[0])), (Node)c.value, null, null);
					   /*      */           
					   /* 4458 */           if (path_violated) {
					   /* 4459 */             violations_counter++;
					   /*      */           }
					   /* 4461 */           Object n = (ObjectCursor)localIterator5.next();
...
/*      */         
					...
					while(localIterator7.hasNext() && localIterator5.hasNext())
					{
						...
						/*      */               {
						/* 4473 */                 if ((((Constraint)((ObjectCursor)cpn).value).getBodyList().contains(((Node)((ObjectCursor<Node>)n).value).getNomeAttivita().split("#")[0])) && (((Constraint)((ObjectCursor)cpn).value).getHeadList().contains(ny.getNomeAttivita().split("#")[0])))
						/*      */                 {
						/* 4475 */                   violations_counter++; } }
						/*      */             }
						/*      */           }
						/* 4478 */           Object nn = (ObjectCursor)localIterator5.next();
...
/*      */           }
/* 4562 */           n = (ObjectCursor)localIterator5.next();
...
/*      */ 
	...
	/*      */             }
	/*      */           }
	/* 4577 */           ObjectCursor<Node> nn = (ObjectCursor<Node>) localIterator6.next();
...
/*      */ 
/*      */ 
/* 4639 */           String nome_attivita = activity.getAttributes().get("concept:name").toString();
/*      */           
...
/* 4684 */       if (mapOri.allocated[i] != false) {
...
/* 4698 */       if (g.getMap().allocated[i] != false) {
...
/*      */         
/* 5455 */         String body = (String)localIterator3.next();
/*      */         
...
/* 5647 */       if (traccia_attivita.allocated[it1] != false) {
...
/* Location:              /home/menick/Scrivania/ProcessMining/Dipendenze/CNMining.jar!/org/processmining/plugins/core/CNMining.class
